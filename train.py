...
def createDataset(filenames, pkl:bool=True): #Recheck this
    if pkl: #load from pkl files
        num_samples = min(int(get_config()["num_samples"]), 69961)
        with open(filenames[0], 'rb') as fp: ds = pickle.load(fp)[:num_samples] #dataset
        ...
        notes_raw = []
        num_files = min(int(get_config()["num_files"]), 1282)
        for f in tqdm(filenames[:num_files],desc=f"Loading files from dataset..."): #Later: Get data from premade .pkl file
...
def split_array_at_count(arr, target_count, target_element):
    count_a = 0
    split_index = 0
    ...
    for i, element in enumerate(arr):
        ...
        if count_a == target_count:
            split_index = i + 1
            break

    return arr[:split_index]

def arrangeSequence(filename, notes:int, start:int):
    ...
    return {"input":input, "src_seq":fileNotes}
...
def train_model(config):
    ...
    for epoch in range(initial_epoch, config["num_epochs"]):
        ...
        for batch in batch_iterator:
            ...
            #summary(model, input_data=inputs, dtypes=[torch.long],verbose=1)
        ...
        if (delete_filename := get_weights_file_path(config, f"{epoch-3:02d}")) and pathlib.Path(delete_filename).exists(): pathlib.Path(delete_filename).unlink()
        torch.save({
            'epoch': epoch,
...
def getOutput(filename:str, notes:int, start:int): #filename of midi file to send into transformer, mode, number of bars to generate
    config = get_config()
    ...
    model.eval()
    arranged_file = arrangeSequence(filename, notes, start) #Only up to max seq_len for now
    with torch.no_grad():
        model_out = top_k_decode(model, arranged_file["input"], config, device, 5)
