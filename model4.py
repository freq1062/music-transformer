...
class InputEmbeddings(nn.Module):
    ...
    def forward(self, x):
        # Scale by sqrt(d_model) so the embedding variance is 1.0
        x = self.embedding(x) * math.sqrt(self.d_model)
        # Apply dropout to the starting "image" of the sequence
...
class RelativeGlobalAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        attn = attn.masked_fill(self.mask[:, :, :seq_len, :seq_len], float("-inf")) # type: ignore
...
class DecoderBlock(nn.Module):
    ...
    def __init__(self, d_model, num_heads, max_len, d_ff, dropout):
        super().__init__()
        self.self_attn = RelativeGlobalAttention(d_model, num_heads, max_len, dropout)
...
class MusicTransformer(nn.Module):
    ...
    def __init__(self, vocab_size, d_model:int=512, num_heads:int=8, max_len:int=200, depth:int=6, d_ff:int=2048, dropout:float=0.1):
        super().__init__()
        self.vocab_size = vocab_size #Number of embeddings to create
        self.d_model = d_model
        self.embed = InputEmbeddings(d_model, vocab_size, dropout)
        self.to_scores = nn.Linear(d_model, vocab_size) #For final probabilities
        # Above is the projection layer i just realized
        
        decoder_blocks = []
        for _ in range(depth):
            decoder_blocks.append(DecoderBlock(d_model, num_heads, max_len, d_ff, dropout))
